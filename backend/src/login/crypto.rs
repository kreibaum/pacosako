...
/// Encrypts a session key using a secret key. This secret key should come from configuration.
pub fn encrypt_session_key(
    ...
    let encrypted = cipher.encrypt(&nonce, session_key.0.as_bytes())?;
...
fn decode_base64(bytes: &str) -> Result<Vec<u8>, anyhow::Error> {
...
fn split_nonce(combined: &str) -> Result<(Vec<u8>, Vec<u8>), anyhow::Error> {
    ...
    let Some(nonce) = split.next() else {anyhow::bail!("No nonce")};
    let Some(payload) = split.next() else {anyhow::bail!("No payload")};
