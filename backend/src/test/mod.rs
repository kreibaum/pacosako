...
async fn paco_2_performance() {
    ...
    let games = remove_short_games(games);
...
async fn load_and_filter_matches(config: EnvironmentConfig) -> Vec<SynchronizedMatch> {
    // Setup database
...
fn remove_short_games(mut games: Vec<SynchronizedMatch>) -> Vec<SynchronizedMatch> {
